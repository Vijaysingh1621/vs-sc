...
export class SansDetector {
    ...
    async detect(filePath: string, content: string, language: string): Promise<Vulnerability[]> {
    ...
    private getSeverity(vulnType: string): 'low' | 'medium' | 'high' | 'critical' {
    ...
    private getTitle(vulnType: string): string {
    ...
    private getDescription(vulnType: string): string {
    ...
    private getCWE(vulnType: string): string {
    ...
    private isFixable(vulnType: string): boolean {
        const fixableTypes = [
            'hardcoded-credentials',
            'info-exposure',
            'weak-crypto',
